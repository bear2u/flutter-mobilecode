  ...
  const walk = (current: string, depth: number) => {
    ...
    if (framework === "flutter") {
      for (const platform of ["ios", "android"] as const) {
        ...
        if (!names.has(platform) || found.has(platform)) continue
        found.set(platform, { platform, directory: current, root: current, framework, needsPrebuild: false })
      }
      return
    }
...
function detectFramework(directory: string, names: Set<string>): Framework {
  if (names.has("pubspec.yaml")) {
    try {
      const spec = parse(readText(path.join(directory, "pubspec.yaml")))
      if (spec?.dependencies?.flutter?.sdk === "flutter") return "flutter"
    } catch {}
  }
...
export function preflight(project: Project, env?: Record<string, string>): string | undefined {
  ...
  if (project.framework === "flutter") {
    if (!commandExists("flutter", searchPath))
      return "Flutter SDK was not found. Add flutter/bin to PATH and try again."
    return
  }
