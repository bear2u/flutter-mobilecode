...
import { DeviceFlutter } from "./device-flutter"
...
type ActiveBuild = {
  flutter?: ReturnType<typeof DeviceFlutter.run>
  state: BuildState
  process?: ChildProcess
  cancelled: boolean
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const runApp = Effect.fn("DevicePreview.runApp")(function* (input: {
      ...
      if (input.action) {
        if (!existing?.flutter || existing.state.status !== "running") return yield* info(input)
        existing.state.status = "building"
        existing.state.step = input.action === "reload" ? "Hot reload" : "Hot restart"
        existing.state.error = undefined
        void existing.flutter
          .restart(input.action === "restart")
          .then(() => {
            if (existing.cancelled) return
            existing.state.status = "running"
            existing.state.step = undefined
          })
          .catch((error) => {
            if (existing.cancelled) return
            existing.state.status = "running"
            existing.state.step = undefined
            existing.state.error = error instanceof Error ? error.message : String(error)
            push(existing.state.log, existing.state.error)
          })
        return yield* info(input)
      }
      if (existing?.flutter) yield* Effect.promise(() => halt(existing))
...
async function execute(
  ...
  try {
    ...
    if (project.framework === "flutter") {
      active.state.directory = project.root
      const device = await runtime.ensureDevice(platform, project.root, env, active, report)
      if (active.cancelled) return
      if (!device) return fail("No simulator or emulator is available. Start a device and try again.")
      active.device = device
      active.state.target = device
      const flutter = DeviceFlutter.run({
      ...
      active.flutter = flutter
      active.process = flutter.child
      const code = await flutter.exit
      active.process = undefined
      if (active.cancelled) return
      if (code !== 0 || active.state.status !== "running")
        return fail(
          buildError(active.state.log) ?? "Flutter exited before the app was ready. Open the log for details.",
        )
      active.state.status = "idle"
      active.state.step = undefined
      return
