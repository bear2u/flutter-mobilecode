...
const preview = Layer.succeed(
  ...
  DevicePreview.Service.of({
    ...
    runApp: (input) => Effect.sync(() => (calls.push(`${input.action ?? "run"} ${input.platform}`), advance())),
...
describe("DeviceRunTool", () => {
  it.effect("reloads a running Flutter app through the registered tool", () =>
    Effect.gen(function* () {
      const detected = { platforms: ["android"] as const, framework: "flutter" as const, servers: [] }
      reset(
        { ...detected, builds: [build("android", "running")] },
        { ...detected, builds: [build("android", "running", { log: ["Reloaded 1 library"] })] },
      )
      const registry = yield* ToolRegistry.Service
      const result = yield* settleTool(registry, call({ action: "reload", platform: "android" }))
      expect(calls).toEqual(["reload android"])
      expect(result.output?.structured).toMatchObject({ framework: "flutter", complete: true })
    }),
  )
