...
function DevicePane(props: {
  ...
  const log = createMemo(() => {
    const current = build()
    const fromBuild =
      current && (current.framework === "flutter" || deviceBuildBusy(current) || current.status === "failed")
  ...
  const progress = createMemo(() => {
    const current = build()
    if (current?.error) return { text: current.error, failed: true }
  ...
  return (
    <div class={`flex flex-col h-full min-h-0 min-w-0 ${props.class ?? ""}`} classList={props.classList}>
      <div class="flex flex-wrap items-center gap-2 px-3 py-2 min-h-10 shrink-0 border-b border-border-weaker-base">
        <Show
        ...
        <Show when={build()?.framework === "flutter"}>
          <Button
            size="small"
            disabled={build()?.status !== "running"}
            onClick={() => void props.device.reloadApp(props.platform, "reload")}
          >
            {language.t("session.device.flutter.reload")}
          </Button>
          <Button
            size="small"
            disabled={build()?.status !== "running"}
            onClick={() => void props.device.reloadApp(props.platform, "restart")}
          >
            {language.t("session.device.flutter.restart")}
          </Button>
        </Show>
        <Show when={build()?.framework === "flutter" || build()?.status === "failed" || deviceBuildBusy(build())}>
