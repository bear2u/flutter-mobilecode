...
export function createDevicePreviewApi(input: { server: ServerConnection.HttpBase; fetch?: typeof globalThis.fetch }) {
  ...
  return {
    ...
    reloadApp: (directory: string, platform: DevicePreview.Platform, action: "reload" | "restart") =>
      request("/api/device-preview/run", directory, { method: "POST", body: JSON.stringify({ platform, action }) }),
