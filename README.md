...
### Flutter (local extension)

Open a Flutter app containing `pubspec.yaml` and its `ios/` or `android/` directories.
The Device tab detects Flutter before the generated native projects. Play starts
`flutter run --machine` on each detected platform, including Flutter's automatic
dependency resolution. Flutter must be on the server's PATH; Xcode and the Android
SDK/emulators are still required for their respective platforms.

The debug process stays connected. **Hot Reload** applies Dart edits while keeping
state; **Hot Restart** restarts the Dart app. **Reload** refreshes only the preview
frame. The toolbar's square button stops the running apps; the pane's Stop button
stops the preview stream. Logs include Flutter build and runtime output.

Agents can use `device_run` with `run`, `status`, `stop`, `reload`, or `restart`.
Reload/restart require a running Flutter app. This extension does not add automated
screen inspection, UI interaction, flavor/entrypoint selection, or physical-device
preview. Existing native, Expo and React Native flows remain available.

For the local web UI, start the backend and frontend in separate terminals:

```bash
bun run --cwd packages/opencode src/index.ts serve --hostname 127.0.0.1 --port 4096
# Run with Node >=22.12 on PATH (the Vite executable uses Node):
bun --cwd packages/app dev -- --host 127.0.0.1 --port 4444
```

Open http://127.0.0.1:4444, add your Flutter project, and open a session to see Device.
